...
public class MovementBehavior : MonoBehaviour
{
	...
	private Vector3 targetPos, currentPos, currentVelocity, finalSteering;
	...
	void FixedUpdate ()
	{
		...
		if (thisBehavior == Behavior.Seek)
			finalSteering += Seek ();
		else if (thisBehavior == Behavior.Flee)
			finalSteering += Flee ();
		else if (thisBehavior == Behavior.Arrive)
			finalSteering += Arrive ();
