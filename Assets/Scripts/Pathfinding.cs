...
public class Pathfinding : MonoBehaviour
{
    ...
    void Update()
    {
        if (!calculatePerFrame)
        {
            ...
            if (previousTargetNode != null)
            {
                ...
                {
                    if (separatePathCalcs)
                    {
                        if (seekerIndex == seekers.Length)
                            seekerIndex = 0;
                        CalculatePath(seekerIndex);
                        seekerIndex++;
                    }
                    else
                    {
    ...
    int GetDistance(Node nodeA, Node nodeB)
    {
        ...
        //14 for diagonal movement cost (14 from sqrt(200))
        //10 for vertical or horizontal movement cost 
        if (distX > distY)
